...
def set_configuration(config):
    ...
    if "ImageName" in config:
        ...
        config_utils.logger.warning(
            "Using default image name: {}".format(
                config_utils.DEFAULT_IMAGE_NAME)
        )
...
def run_inference(new_config, config_changed):
    ...
    # config_utils.SCHEDULED_THREAD = threading.Timer(int(new_config["prediction_interval_secs"]), run_inference, [new_config, config_changed])
    config_utils.SCHEDULED_THREAD = threading.Timer(
        5, run_inference, [new_config, config_changed])
